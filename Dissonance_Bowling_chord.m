function Dissonance_Bowling_chord(cpn, dur, rmp, spl, nrep, note_shift, BMF)
...
F0s = round(220*2^(note_shift/12)*2.^([1:12]/12),0); % Bowling freqs
...
chord_comb = [7 12; 4 7;3 7;5 7;2 7;4 10;3 6;4 8;1 6];

% For Chord
% power chord, major triad, minor triad, suspeded 4th, suspended 2nd, Italian 6th, diminished triad,
%augmented triad, Viennese trichord
% 0 7 12, 0 4 7, 0 3 7, 0 5 7, 0 2 7, 0 4 10, 0 3 6, 0 4 8, 0 1 6
for ichord = 1:9
   pin2 = complex_tone(dur,rmp,F0s(chord_comb(ichord,1)),cpn,filter_type,Wn_freq,include_fundmntl,spl,Fs);
   pin3 = complex_tone(dur,rmp,F0s(chord_comb(ichord,2)),cpn,filter_type,Wn_freq,include_fundmntl,spl,Fs);
   pin = pin_A3 + pin2 + pin3; % compute 2nd complex tone and add to A3
...
std_mean_interval_BE(ichord) = std(mean_ic_sout_BE);
std_std_interval_BE(ichord) = std(std_ic_sout_BE);
std_mean_interval_BS(ichord) = std(mean_ic_sout_BS);
std_std_interval_BS(ichord) = std(std_ic_sout_BS);
...
chords = 1:9;
chords_names = ["power chord", "major triad", "minor triad", "suspeded 4th"...
    "suspended 2nd", "Italian 6th", "diminished triad", "augmented triad",...
    "Viennese trichord"]; % for bowling plot
plot(chords,std_std_interval_BE,'-*b','linewidth',1.5);
hold on
plot(chords,std_std_interval_BS,'-*r','linewidth',1.5);
%chord_2=1:3:12;
...
load('Bowling_Fig2_data.mat','x','y','y_norm','h')
std_interval_BE_norm = (std_mean_interval_BE-min(std_mean_interval_BE))/(max(std_mean_interval_BE)-min(std_mean_interval_BE))*(max(y_norm)-min(y_norm))+min(y_norm);  % normalize the std values to the maximum value
std_interval_BS_norm = (std_mean_interval_BS-min(std_mean_interval_BS))/(max(std_mean_interval_BS)-min(std_mean_interval_BS))*(max(y_norm)-min(y_norm))+min(y_norm);  % normalize the std values to the maximum value
%subplot(2,1,2)
figure
%plot(chords,std_interval_BE_norm - mean(std_interval_BE_norm) + 0.5,'-*b','linewidth',1.5); % plot normalized std values, referenced to the mean std
plot(chords,std_interval_BE_norm,'-*b','linewidth',1.5);
hold on
%plot(chords,std_interval_BS_norm - mean(std_interval_BS_norm) + 0.5,'-*r','linewidth',1.5); % plot normalized std values, referenced to the mean std
plot(chords, std_interval_BS_norm,'-*r','linewidth',1.5);

plot(y_norm,'o-k','linewidth',0.5); % could plot Freq ratios (stored in loot.x)
%plot(h+0.25,'o--k','linewidth',0.5); % could plot Freq ratios (stored in loot.x)
plot([1 length(chords)],[0.5, 0.5],':k')
%chord_2=1:3:12;
%plot(chord_2,std_chord,'-*','linewidth',1.5);
%hold off
ylabel('Conssonance Score (norm)');
xticklabels(chords_names);
xlabel('Chors No.')
...
%legend('Model BE','Model BS','Bowling','Harmonic Similarity')

% Compute correlations between Tufts data and model std's 
[diss_corr_BE_std, p_BE_std] = corrcoef(y,std_std_interval_BE)
[diss_corr_BS_std, p_BS_std] = corrcoef(y,std_std_interval_BS)
[diss_corr_BE_mean, p_BE_mean] = corrcoef(y,std_mean_interval_BE)
[diss_corr_BS_mean, p_BS_mean] = corrcoef(y,std_mean_interval_BS)
...
% CFs=logspace(log10(430),log10(1600),30);
