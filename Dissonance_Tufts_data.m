function [diss_corr_BE_std, p_BE_std, diss_corr_BE_mean, p_BE_mean, diss_corr_BS_std, p_BS_std, diss_corr_BS_mean, p_BS_mean] = Dissonance_Tufts_data(cpn, dur, rmp, spl, nrep, note_shift, BMF)
%% Entire "experiment" is run with this function    (adapted from ARO 2019 poster)
% Musical Dissonance Neural coding on the fluctuation profile of the IC cells
% from SFIE model 

% Note that "Tufts_Fig5_data.mat" file is needed for plot that compares model to data.
...
% note_shift: note shift (modulation) of F0s from A3
%             E.g., note_shift=2 means root note shifts to B3;
% BMF: Best modulation frequency for IC model cell, around 100 Hz;
% 
%% Stimulus Generation
...
F0s = 220*2^(note_shift/12)*2.^((0:25)/24); % T&L freqs
...
[pin_A3] = complex_tone(dur,rmp,F0s(1),cpn,0,0,1,spl,Fs);  % pressure wave (pressure 'input') for standard complex tone, A3
...
for ifreq = 1:length(F0s)
    ...
    pin = 20e-6 * 10.^(spl/20) * pin/rms(pin); % convert to pascal
    [mean_ic_sout_BE,std_ic_sout_BE,mean_ic_sout_BS,std_ic_sout_BS,CFs] = model_IC_LHC(pin, BMF, Fs, nrep);  % this function is included in this file, see below.
   
    %% IC model
    % compute metrics
    std_mean_interval_BE(ifreq) = var(mean_ic_sout_BE);
    std_std_interval_BE(ifreq) = var(std_ic_sout_BE);
    std_mean_interval_BS(ifreq) = var(mean_ic_sout_BS);
    std_std_interval_BS(ifreq) = var(std_ic_sout_BS);
...
hold on
plot(intervals,std_std_interval_BE,'-*r','linewidth',1.5);
ylabel('Standard Deviation');
ax = gca;
ax.FontSize = 18;

std_interval_BE_norm = std_mean_interval_BE/max(std_mean_interval_BE);  % normalize the std values to the maximum value
std_interval_BS_norm = std_mean_interval_BS/max(std_mean_interval_BS);  % normalize the std values to the maximum value
%subplot(2,1,2)
figure
...
load('Tufts_Fig5_data','loot');
plot(loot.y,'o-k','linewidth',0.5); % could plot Freq ratios (stored in loot.x)
...
ax = gca;
ax.FontSize = 18;
legend('Model BE','Model BS','Tufts')

% Compute correlations between Tufts data and model std's 
[diss_corr_BE_std, p_BE_std] = corrcoef(loot.y,std_std_interval_BE);
[diss_corr_BE_mean, p_BE_mean] = corrcoef(loot.y,std_mean_interval_BE);
[diss_corr_BS_std, p_BS_std] = corrcoef(loot.y,std_std_interval_BS);
[diss_corr_BS_mean, p_BS_mean] = corrcoef(loot.y,std_mean_interval_BS);
end
